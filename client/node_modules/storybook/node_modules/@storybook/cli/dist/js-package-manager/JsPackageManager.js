...
class JsPackageManager {
  ...
  addStorybookCommandInScripts(options) {
    ...
    const noDll = '--no-dll';
    const storybookCmd = (options === null || options === void 0 ? void 0 : options.staticFolder) ? `start-storybook -p ${sbPort} -s ${options.staticFolder} ${noDll}` : `start-storybook -p ${sbPort} ${noDll}`;
    const buildStorybookCmd = (options === null || options === void 0 ? void 0 : options.staticFolder) ? `build-storybook -s ${options.staticFolder} ${noDll}` : `build-storybook ${noDll}`;
